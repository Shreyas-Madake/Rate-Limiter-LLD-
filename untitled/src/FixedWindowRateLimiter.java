...
public class FixedWindowRateLimiter extends RateLimiter {
    ...
    public boolean allowRequest(String userId) {
        ...
        long currentReqWindow = System.currentTimeMillis() / (config.getWindowInSeconds() * 1000L);

        requestCount.compute(userId, (id, count) -> {
            long lastReqWindow = windowStart.getOrDefault(id, 0L);

            if (lastReqWindow != currentReqWindow) {
                // window expired -> reset counter and window start time
